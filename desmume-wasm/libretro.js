...
function selectSaveFiles(files)
{
   ...
   for (var i = 0; i < count; i++)
   {
      ...
      filereader.file_name = files[i].name;
      filereader.readAsArrayBuffer(files[i]);
      filereader.onload = function(){uploadSaveData(this.result, this.file_name)};
      filereader.onloadend = function(evt)
      {
         console.log("WEBPLAYER: file: " + this.file_name + " upload complete");
         if (evt.target.readyState == FileReader.DONE)
...
function uploadSaveData(data,name)
{
   var dataView = new Uint8Array(data);
   Module.FS.createDataFile('/', name, dataView, true, false);

   var data = Module.FS.readFile(name,{ encoding: 'binary' });
   Module.FS.writeFile('/home/web_user/retroarch/userdata/saves/DeSmuME/' + name, data ,{ encoding: 'binary' });
   Module.FS.unlink(name);
}
...
async function savedir() {
      Module.FS.mkdir('/home/web_user/retroarch/userdata/saves/DeSmuME/')
}
